...
show_puuid_progress() {
    local current=$1
    local total=$2
    printf "\rProcessing PUUID: %d/%d" "$current" "$total"
}
...
show_match_progress() {
    ...
    printf "Current match: %s | Total: %s | Remaining: ~%s" \
           "$(format_time $elapsed)" \
           "$(format_time $((total_duration + elapsed)))" \
...
for puuid in "${PUIDS[@]}"; do
    counter=$((counter + 1))
    show_puuid_progress $counter $total_puuid_count
    ...
    for match_id in "${match_array[@]}"; do
        ...
        tput sc
        printf "\nProcessing match %d/%d: %s" "$processed_matches" "$total_matches" "$match_id"
        ...
        echo "$info" | jq -c '.participants[]' | while read -r participant; do
         ...
         # Calculate times
        match_duration=$(( $(date +%s) - match_start ))
        total_duration=$(( total_duration + match_duration ))
        average_duration=$(( total_duration / processed_matches ))
        remaining_matches=$(( total_matches - processed_matches ))
        remaining_time=$(( average_duration * remaining_matches ))
        
        # Restore cursor position and update progress
        tput rc
        tput ed
        show_match_progress $processed_matches $total_matches $match_duration $remaining_time
    done

    tput cuu1  # Move up to clear last progress line
    echo -e "\nFinished processing $total_matches matches for PUUID: $puuid"
    echo "Total time: $(format_time $total_duration) | Avg per match: $(format_time $average_duration)"
    
    # Rate limit handling
    if ((counter % MAX_REQUESTS_PER_2_MINUTES == 0)); then
        echo "Rate limit threshold reached, sleeping for $WAIT_TIME_2MIN seconds..."
