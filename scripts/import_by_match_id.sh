...
if [ $# -eq 0 ]; then
    echo "Error: No match ID provided"
...
process_match() {
    ...
    if [ -z "$match_data" ] || [ "$match_data" = "null" ]; then
        echo "Error: Could not fetch data for match $match_id"
        exit 1
    fi
    ...
    echo "Successfully processed match: $match_id"
}

# Start processing
process_match "$MATCH_ID"

echo "Data import completed for match ID: $MATCH_ID"