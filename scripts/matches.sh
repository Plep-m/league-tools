...
export API_KEY DB_HOST DB_USER DB_PASS DB_NAME

# Rate limit constants (adjusted for parallel)
MAX_PARALLEL_JOBS=4
REQUEST_DELAY=1.25

# Time tracking setup
export TOTAL_MATCHES=0
export PROCESSED=0
START_TIME=$(date +%s)
...
# Progress display coprocess
coproc PROGRESS_DISPLAY {
    while true; do
        read -t 1 -u 3 processed total
        [ -n "$processed" ] && PROCESSED=$processed
        [ -n "$total" ] && TOTAL_MATCHES=$total
        
        elapsed=$(( $(date +%s) - START_TIME ))
        if [ $TOTAL_MATCHES -gt 0 ] 2>/dev/null; then
            percent=$((PROCESSED * 100 / TOTAL_MATCHES))
            filled=$((percent * TERM_WIDTH / 100))
            remaining=$(( (elapsed * (100 - percent) ) / (percent + 1) ))
        else
            percent=0
            filled=0
            remaining=0
        ...
        empty=$(printf "%$((TERM_WIDTH - filled))s")
        
        printf "\r[%s%s] %d%% | Elapsed: %s | Remaining: %s | Processed: %d/%d" \
               "$bar" "$empty" "$percent" \
               "$(format_time $elapsed)" \
...
process_match() {
    ...
    response=$(curl -s -H "X-Riot-Token: $API_KEY" \
        "https://europe.api.riotgames.com/lol/match/v5/matches/$match_id")
    
    if echo "$response" | jq -e '.status_code == 401' >/dev/null; then
        echo "API Error: Invalid API Key" >&2
        return 1
    fi
    
    if [ -z "$response" ]; then
        echo "Empty response for match $match_id" >&2
        return 1
    fi

    # Extract metadata and info
    metadata=$(echo "$response" | jq '.metadata')
    info=$(echo "$response" | jq '.info')
    ...
    result=$([ "$result" = "true" ] && echo "win" || echo "lose")
    ...
    VALUES ('$game_id', '$game_date', '$result', $duration, $queue_id, 
            '$(mysql_escape "$game_version")', '$(mysql_escape "$game_mode")', '$(mysql_escape "$game_type")')
    ...
    echo "$info" | jq -c '.teams[]' | while read -r team; do
        ...
        if [ -z "$team_id" ]; then
            echo "Error: Could not find team_id for game_id=$game_id and team_id_api=$team_id_api" >&2
            continue
        fi
    ...
    echo "$((PROCESSED + 1)) $TOTAL_MATCHES" >&3
    return 0
}
export -f process_match

# Main execution
match_ids=$(curl -s -H "X-Riot-Token: $API_KEY" \
    "https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/$PUUID/ids?queue=440&start=0&count=100")

readarray -t match_array <<< "$(echo "$match_ids" | jq -r '.[]')"
TOTAL_MATCHES=${#match_array[@]}
echo "0 $TOTAL_MATCHES" >&3  # Initial state

# Process matches with parallel
printf "%s\n" "${match_array[@]}" | parallel -j$MAX_PARALLEL_JOBS --delay $REQUEST_DELAY \
    --joblog joblog.txt \
    --progress --bar \
    process_match {}

# Cleanup
kill $PROGRESS_DISPLAY_PID 2>/dev/null
rm progress_pipe
echo -e "\nTotal processing time: $(format_time $(($(date +%s) - START_TIME)))"